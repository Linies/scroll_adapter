...
class _GestureDetectorState extends State<GestureDetector> {
  ...
  void onPointerMove(PointerMoveEvent event) {
    ...
    cleanupDoubleTimer();
    final lastTouch = touches.lastWhere((touch) => touch.id == event.pointer);
    ...
    switch (state) {
      ...
      case _GestureState.PointerDown:
        // fix: move event when pointer up
        if (lastTouch.startOffset.dx != event.localPosition.dx &&
            lastTouch.startOffset.dy != event.localPosition.dy) {
          switch2MoveStartState(touch, event);
        }
        break;
...
/// See also:
///
///  * [GestureDetector.onMoveUpdate], which allows callers to be notified of these
///    events in a widget tree.
///  * [GestureDetector.onMoveStart], which allows callers to be notified for the first time this event occurs
///  * [GestureDetector.onMoveEnd], which allows callers to be notified after the last move event occurs.
@immutable
...
/// See also:
///
///  * [GestureDetector.onScaleUpdate], which allows callers to be notified of these
///    events in a widget tree.
@immutable
class ScaleEvent {
...
/// would create a [GestureDetector.ScrollEvent]
///
